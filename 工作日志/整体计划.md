...
Phase 4 (Day 6-8)    深入 Agent 图     ← 替换预构建为手动 LangGraph，自定义 ReAct 循环  ✅ 已完成
...
**方式**: `SqliteSaver`（持久化 SQLite）。

**内容**:
- `agent/memory/checkpointer.py` — SqliteSaver 配置
- `thread_id` 管理 — 支持 `/thread <id>` 切换、`clear` 新建
- 消息窗口限制（MAX_MESSAGES=10），长对话 token 省 50-80%
- `scripts/inspect_db.py` — 数据库检查脚本

---

## Phase 4 — 深入 Agent 图 ✅

**目标**: 将 `create_agent` 替换为手动构建的 LangGraph 状态图，完全理解和掌控 ReAct 循环。

**意义**: 从"会用"到"会改"的关键一步。

**内容**:
- 新建 `agent/graph.py` — 手动定义 `AgentState`、`StateGraph`
- 6 个节点: `trim`、`rewrite`、`agent`、`tools`、`increment`、`force_reply`
- 自定义 `AgentState`（增加 `iteration_count` 字段）
- 查询改写节点（`rewrite_node`）— 短问题/含代词时 LLM 改写，提高 RAG 命中率
- 工具调用上限保护（`max_iterations=5`）— 防止无限循环，超限强制回答
- `main.py` 支持 `--classic` 参数保留旧版，默认使用新版 StateGraph
